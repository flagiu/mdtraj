...
class data:
    ...
    for keyword in hkeywords:
        if keyword in self.headers:
            if keyword == "xlo xhi" or keyword == "ylo yhi" or keyword == "zlo zhi":
                ...
                f.write(f"{pair[0]} {pair[1]} {keyword}\n")
            elif keyword == "xy xz yz":
                triple = self.headers[keyword]
                f.write(f"{triple[0]} {triple[1]} {triple[2]} {keyword} \n")
            else:
                f.write(self.headers[keyword],keyword,'\n')
    for pair in skeywords:
        keyword = pair[0]
  ...
  def viz(self,isnap):
    ...
    try:
        triple = self.headers["xy xz yz"]
    except KeyError:
        triple = [0.0,0.0,0.0]
    box = [xlohi[0],ylohi[0],zlohi[0],xlohi[1],ylohi[1],zlohi[1],triple[0],triple[1],triple[2]]
