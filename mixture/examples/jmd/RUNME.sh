...
tag=$1 # string tag for output files (can be the empty string: '' or "")
...
rm traj.jmd; ls ../configurations/pos_* | sort -V | while read el; do cat $el >> traj.jmd; done
MDTRAJ="${MDTRAJ_PATH}/bin/mdtraj -jmd traj.jmd -fskip $f0 $f1 -tag '$tag'" # here the '' is crucial for empty tags!

echo "|||||||| $tag : g(r) and S(q) and .xyz ..."
...
rm *.pdf log.$tag nnd.${tagdot}dat traj.jmd coordnum.${tagdot}dat ed_q.${tagdot}dat
rm boo*${tagdot}dat boc*${tagdot}dat boo*${tagdot}local_ave boc*${tagdot}local_ave
}

## example: whole trajectory
run_fraction_of_trajectory "" 0.01 0.0

## example: different fractions of the trajectory
#run_fraction_of_trajectory liquid 0.01 0.5
#run_fraction_of_trajectory crystal 0.5 0.0
