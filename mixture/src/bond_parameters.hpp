...
class Bond_Parameters
{
  ...
  public:
    ...
    void compute(int timestep, vector<ptype> ps, bool debug)
    {
      ...
      for(k=0;k<nb_list->bond_list[u].size();k++){
        Cl_ij.set(k, 0.0);
        i = nb_list->int2i( nb_list->bond_list[u][k], N );
      ...
      for(i=0;i<N;i++){
        for(k=0;k<ps[i].neigh_list[u].size();k++){
          ...
          fval = nb_list->fcut( rijSq/nb_list->rcutSq[u][t] );
          ql_dot[i] += fval*Cl_ij[k] / nb_list->neigh_anytype[u][i];
          ql_dot[j] += fval*Cl_ij[k] / nb_list->neigh_anytype[u][j];  // take into account j>i
        }
      }
